...
else if (returnCode = 2)
{
  if (abbr = "")
  {
    msgbox,,,Nothing cannot be deleted!, 1
    Goto, start
  }
  abbrExist := f_checkForExisting()
  if abbrExist = 0
  {
    MsgBox, 0,, Entry %abbr% doesn't exist, and therefore cannot be deleted?
    Goto, start
  }
  else
  {
...
run, "C:\Users\ksj\OneDrive - Sirris\ksj_persoonlijk\PortableApps\AutoHotkey_1.1.35.00\Template.ahk"
...
f_createGUI() 
{
  ...
  Gui, Add, Button, gButtonDEL, DEL ; The label ButtonDEL (if it exists) will be run when the button is pressed.
